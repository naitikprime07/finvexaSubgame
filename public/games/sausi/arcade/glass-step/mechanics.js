export default class MechanicsScene extends Phaser.Scene {
  ...
  create() {
    ...
    // ðŸ”§ Initialize game state values
    ...
    this.input.on("gameobjectdown", (pointer, obj) => {
      ...
      if (obj === currentPair.left || obj === currentPair.right) {
        if (obj.safe) {
          ...
          this.jumpSound.play();
          // ðŸ‘‡ Set jump frame based on side
          ...
          this.sys.tweens.add({
            ...
            onComplete: () => {
              this.currentStep++;
              this.player.setFrame(1); // ðŸ‘ˆ Back to standing frame after jump
  ...
  showInstructions() {
    ...
    this.descriptionText = this.add
      .text(
        540,
        800,
        "Tap a platform to jump forward.\nEach step has one safe and one fake glass tile â€” only one will hold your weight! Choose wisely and reach the top before time runs out. One wrong step, and it's game over! ðŸ’¥",
        {
          font: "60px Outfit",
          color: "#ffffff",
