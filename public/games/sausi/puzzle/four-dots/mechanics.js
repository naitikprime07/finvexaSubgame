export default class MechanicsScene extends Phaser.Scene {
  ...
  preload() {
    ...
    const loadFromCfg = (cfg) => {
      ...
      this.load.script("webfont", "/game-vendor/webfont-1.6.26.js");
      this.load.start();
    };
  ...
  create() {
    ...
    // âœ… Background: you have it under images2 with key "background"
    ...
    // ðŸ”’ Hide gameplay visuals until Play is tapped
  ...
  drawGrid() {
    ...
    return graphics; // â¬…ï¸ now returns for container use
  }
  ...
  dropToken(row, col, player, callback) {
    ...
    // ðŸ”Š play tap sfx as it starts coming down (if loaded in config as "tap")
