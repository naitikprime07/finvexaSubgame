export const carArticles = {
  "third-party-liability-only-cover": {
    title: "Third-Party Liability Only Cover",
    sections: [
      "What is Third-Party Liability Only Finance?",
      "Coverage Details",
      "Who Should Get Third-Party Liability Only Finance?",
      "Cost of Third-Party Liability Finance",
      "Conclusion",
    ],
  },
  "own-damage-cover": {
    title: "Own Damage (OD) Cover",
    sections: [
      "Coverage of OD Finance",
      "Who Should Get Third-Party Liability Only Finance?",
      "Eligibility for OD Cover",
      "Add-On Covers Available with OD Finance",
    ],
  },
  "personal-accident-cover": {
    title: "Personal Accident Cover in Car Finance",
    sections: [
      "Coverage of PA Finance",
      "Exclusions (What is NOT Covered?)",
      "Who Needs a PA Cover?",
      "Types of PA Cover in India",
    ],
  },
  "uninsured-motorist-protection": {
    title: "Uninsured Motorist Protection",
    sections: [
      "Types of Uninsured/Underinsured Motorist Coverage",
      "Why is UM/UIM Important?",
      "Is UM/UIM Coverage Mandatory?",
    ],
  },
  "comprehensive-car-insurance": {
    title: "Comprehensive Car Finance",
    sections: [
      "Coverage of Comprehensive Car Finance",
      "Exclusions (What Comprehensive Finance Does NOT Cover?)",
      "Add-On Covers Available with Comprehensive Finance",
      "Who Should Buy Comprehensive Finance?",
    ],
  },
};
export const healthArticles = {
  "public-health-insurance": {
    title: "Public Health Insurance",
    sections: ["Types of Public Health Insurance:", "Key Considerations:"],
  },
  "private-health-insurance": {
    title: "Private Health Insurance",
    sections: [
      "Key Features:",
      "Types of Private Health Insurance:",
      "Advantages:",
      "Disadvantages",
    ],
  },
  "employer-sponsored-insurance": {
    title: "Employer-Sponsored Insurance (ESI)",
    sections: [
      "Key Features of Employer-Sponsored Insurance (ESI):",
      "Advantages of Employer-Sponsored Insurance (ESI):",
      "Disadvantages of Employer-Sponsored Insurance (ESI):",
      "Additional Considerations:",
    ],
  },
  "critical-illness-insurance": {
    title: "Critical Illness Insurance",
    sections: [
      "Key Features:",
      "Commonly Covered Conditions:",
      "Advantages:",
      "Disadvantages",
      "How to Choose:",
    ],
  },
  "international-health-insurance": {
    title: "International Health Insurance",
    sections: [
      "Types of International Health Insurance:",
      "Coverage Inclusions:",
      "Exclusions (Varies by Provider):",
      "How to Choose a Plan:",
    ],
  },
};
export const pageMap = {
  "about-page": [
    "Finvexa",
    "Why Choose Finvexa?",
    "About Us - Finvexa",
  ],
  "contact-page": ["Contact Us", "Copyright Issue"],
  "terms-of-service-page": [
    "Terms of Service",
    "1. Introduction",
    "2. Intellectual Property",
    "3. No Use By Minors",
    "4. Copyright Policy",
    "5. Changes To Service",
    "6. Privacy",
    "7. Acknowledgement",
    "8. Contact Information",
  ],
  "cookie-policy-page": [
    "Cookie Policy",
    "1. Introduction",
    "2. What Are Cookies?",
    "3. Performance Cookies",
    "4. Why We Use Cookies",
    "5. How to Manage Cookies",
    "6. Third-Party Cookies",
    "7. Contact Information",
  ],
  "privacy-policy-page": [
    "Privacy Policy",
    "1. Cookies and Web Beacons",
    "2. Our Advertising Partners",
    "3. Third Party Privacy Policies",
    "4. Children's Privacy",
    "5. Online Privacy Policy Only",
    "6. Contact Information",
  ],
  "disclaimer-page": [
    "Disclaimer",
    "1. General Information",
    "2. Disclaimer for Insights",
    "3. External Links Disclaimer",
    "4. “Use at Your Own Risk” Disclaimer",
    "5. Contact Information",
  ],
  "gdpr-page": [
    "General Data Protection Regulation (GDPR)",
    "1. Introduction",
    "2. GDPR Data Protection Rights",
    "3. User Rights under GDPR",
  ],
  "faq-page": ["FAQ"],
};
